# {Topic} — Open Questions

> Context: {1–3 sentences: what the project is, its stack, and what this batch of
> questions is about. This block is fed verbatim to the per-question Discuss chat,
> so name the directories where relevant docs/plans live.}

## Decide first

### Q1. {Either/or question title — state the choice in the title?}
{One line: why this needs a decision now.}
*Affects:* {areas, files, or other questions this touches — optional line, delete if unused}
- **Decides:** {what the answer pins down}
- **Recommendation:** Pick: {option} — {one clause why; prefix claims you did not verify with "unverified — "}
- **Other options:** {alternative} — {one tradeoff}. {ruled-out alternative} — {why} (avoid)

### Q2. {Next cross-cutting question?}
{Why line.}
- **Decides:** {…}
- **Recommendation:** Pick: {…} — {…}
- **Other options:** {…}

## {Section name, e.g. a feature area}

### Q3. {Question title?}
{Why line.}
- **Decides:** {…}
- **Recommendation:** Pick: {…} — {…}
- **Other options:** {…}
