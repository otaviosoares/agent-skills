<!--
  recipes.template.md — `init` copies this in as the repo's plans/loop.recipes.md: the ~stable per-repo
  judgment the loop-kit skeleton applies BY NAME (each `## SECTION` answers a `RECIPE → ## NAME` marker).
  Fill EVERY `<<FILL: …>>` by hand before a real run — the kit never auto-fills, and an unresolved
  section makes the loop STOP with LOOP_STATUS=BLOCKED (a confidently-wrong recipe is worse than a blank
  one). Per-wave scope (## TARGET / ## KEYSTONES) → plans/loop.scope.md; the scope label, run-log handle
  and branch prefix are VALUES (WAVE / RUNLOG / BRANCH_PREFIX) in plans/loop.config.sh. Search `<<FILL`.
-->
